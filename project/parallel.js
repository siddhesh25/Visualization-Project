...
function render_range(selection, i, max, opacity) {
  selection.slice(i,max).forEach(function(d) {
    path(d, foreground, color(d.states,opacity));
  });
};
...
function data_table(sample) {
  ...
  table
    .append("span")
      ...
      .style("background", function(d) { return color(d.states,0.85) })
...
function highlight(d) {
  ...
  d3.selectAll(".row").style("opacity", function(p) { return (d.states == p) ? null : "0.3" });
  path(d, highlighted, color(d.states,1));
}
...
function brush() {
  ...
  data
    .filter(function(d) {
      return !_.contains(excluded_groups, d.states);
    })
    .map(function(d) {
  ...
  var tallies = _(selected)
    .groupBy(function(d) { return d.states; })
